        ...
        .concatMap(function(element) {
          return Rx.Observable.fromPromise(element.getSetEntityReference())
            .flatMap(function(enrichedBridgeDbStyleEntityReference) {
              ...
              var biopaxSubjects = _.filter(subject, function(item) {
                return item.indexOf('biopax') > -1;
              });
              var biopaxEntityReferenceTypes = _.filter(enrichedBridgeDbStyleEntityReference.type,
                  function(item) {
                    return item.indexOf('biopax') > -1;
                  });
              var intersection = _.intersection(biopaxSubjects, biopaxEntityReferenceTypes);
              if (intersection.length > 0) {
                enrichedEntityReference.type = intersection[0];
              }
              ...
              console.log('enrichedEntityReference');
              console.log(enrichedEntityReference);
